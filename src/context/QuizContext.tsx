...
import { calculateMatches, calculateQuickPreview } from '../utils/scoringAlgorithm';
import type { UserAnswers, DistroMatch } from '../utils/scoringAlgorithm';
import { checkDealBreakers, getDealBreakerSummary } from '../utils/dealBreakers';
...
export function QuizProvider({ children }: { children: ReactNode }) {
  ...
  const [dealBreakerWarnings, setDealBreakerWarnings] = useState<DealBreakerWarning[]>([]);
  ...
  const cleanOrphanedAnswers = useCallback((
    ...
    tempAnswers.set(newQuestionId, { questionId: newQuestionId, selectedOptions: newSelectedOptions });
  ...
  const setAnswer = useCallback((questionId: string, selectedOptions: string[]) => {
    setAnswersMap(prev => cleanOrphanedAnswers(prev, questionId, selectedOptions));
  ...
  const calculateResults = useCallback(() => {
    ...
    const legacyWarnings: Warning[] = dealBreakers.map(db => ({
      type: db.type === 'game' ? 'gaming' : db.type === 'software' ? 'compatibility' : db.type,
      message: db.message,
  ...
  const getTopResults = useCallback((count: number) => {
