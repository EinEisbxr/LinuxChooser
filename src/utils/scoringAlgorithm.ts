...
export function buildUserProfile(answers: UserAnswers): UserProfile {
    ...
    switch (primaryUse) {
        ...
        case 'server':
            ...
            profile.needsBeginnerFriendly = Math.max(
                0,
                profile.needsBeginnerFriendly - 3,
            );
            break;
        case 'learning':
            profile.wantsToLearn = true;
    ...
    if (secondaryUses) {
        ...
        if (
            secondaryUses.includes('video-editing') ||
            secondaryUses.includes('graphics')
        ) {
    ...
    if (specificGames && !specificGames.includes('none')) {
        ...
        profile.hasProblematicGames = specificGames.some((g) =>
            problematicGameIds.includes(g),
        );
    }
    ...
    const softwareReqs = answers['software-requirements'] as
        | string[]
        | undefined;
    ...
    switch (learning) {
        case 'none':
            ...
            profile.needsBeginnerFriendly = Math.min(
                10,
                profile.needsBeginnerFriendly + 2,
            );
            break;
 ...
 */
export function calculateDistroMatch(
    profile: UserProfile,
    distro: Distro,
): number {
    let score = 0;
    ...
    const beginnerDiff = Math.abs(
        distro.beginnerFriendly - profile.needsBeginnerFriendly,
    );
    ...
    if (profile.needsGaming > 0) {
        const gamingScore =
            distro.gamingSupport * (profile.needsGaming / 10) * WEIGHTS.gaming;
        score += gamingScore;
    ...
    if (profile.needsPrivacy > 5) {
        const privacyScore =
            distro.privacyFocus * (profile.needsPrivacy / 10) * WEIGHTS.privacy;
        score += privacyScore;
    ...
    if (profile.needsProfessional > 5) {
        const profScore =
            distro.professionalUse *
            (profile.needsProfessional / 10) *
    ...
    if (profile.needsGermanCommunity) {
        const communityScore =
            distro.communitySupport * WEIGHTS.communitySupport;
    ...
    if (
        profile.needsProfessional > 7 &&
        distro.targetAudience.includes('developer')
    ) {
        score += 5;
    }
    if (
        profile.experienceLevel < 2 &&
        distro.targetAudience.includes('beginner')
    ) {
        score += 8;
    }
...
export function generateMatchReasons(
    ...
    if (profile.needsBeginnerFriendly > 6 && distro.beginnerFriendly >= 8) {
        reasons.push(
            `${distro.name} ist besonders einsteigerfreundlich und leicht zu bedienen.`,
        );
    }
    ...
    if (profile.needsStability > 7 && distro.stability >= 8) {
        reasons.push(
            `Du legst Wert auf Stabilität - ${distro.name} ist bekannt für seine Zuverlässigkeit.`,
        );
    }
    ...
    if (profile.needsCuttingEdge > 7 && distro.cuttingEdge >= 8) {
        reasons.push(
            `${distro.name} bietet dir immer die aktuellste Software.`,
        );
    }
    ...
    if (profile.needsGaming > 5 && distro.gamingSupport >= 8) {
        reasons.push(
            `Perfekt fürs Gaming - ${distro.name} hat exzellente Spieleunterstützung.`,
        );
    }
    ...
    if (profile.needsPrivacy > 7 && distro.privacyFocus >= 8) {
        reasons.push(
            `${distro.name} legt besonderen Wert auf deine Privatsphäre.`,
        );
    }

    // Old hardware
    if (
        (profile.hasOldHardware || profile.hasLimitedRAM) &&
        distro.performance >= 8
    ) {
        reasons.push(
            `${distro.name} ist ressourcenschonend und läuft auch auf älterer Hardware gut.`,
        );
    }
    ...
    if (
        profile.hasNvidia &&
        ['pop-os', 'nobara', 'manjaro', 'linux-mint'].includes(distro.id)
    ) {
        reasons.push(`Gute NVIDIA-Unterstützung out-of-the-box.`);
    }

    // Desktop environment
    if (profile.prefersWindowsLike && de.windowsLike >= 7) {
        reasons.push(
            `${de.name} bietet ein Windows-ähnliches Layout - ideal für Umsteiger.`,
        );
    }
    if (profile.prefersMacLike && de.macLike >= 7) {
        reasons.push(`${de.name} hat ein elegantes, macOS-ähnliches Design.`);
    }
    if (profile.prefersTiling && de.tilingSupport) {
        reasons.push(
            `${de.name} unterstützt Tiling für maximale Produktivität.`,
        );
    }
    ...
    if (profile.needsCustomization > 7 && distro.customizability >= 8) {
        reasons.push(
            `${distro.name} lässt sich nach deinen Wünschen anpassen.`,
        );
    }

    // German community
    if (profile.needsGermanCommunity && distro.communitySupport >= 8) {
        reasons.push(`Große deutschsprachige Community für Support und Hilfe.`);
    }
    ...
    if (profile.wantsToLearn && profile.experienceLevel >= 3) {
        if (['arch', 'gentoo', 'void', 'nixos'].includes(distro.id)) {
            reasons.push(
                `Ideal zum tiefen Lernen - du verstehst wie alles funktioniert.`,
            );
        }
    }
    ...
    if (profile.needsCuttingEdge > 6 && distro.hasAUR) {
        reasons.push(`Zugang zum AUR mit tausenden zusätzlichen Paketen.`);
    }

    // Flatpak for proprietary software
    if (profile.needsProprietarySoftware && distro.hasFlatpak) {
        reasons.push(`Flatpak-Unterstützung für zusätzliche Software.`);
    }

    // Ensure we have at least one reason
    if (reasons.length === 0) {
        reasons.push(
            `${distro.name} mit ${de.name} ist eine solide Wahl für deine Anforderungen.`,
        );
    }
...
export function generateWarnings(
    ...
    if (profile.experienceLevel < 2 && distro.beginnerFriendly < 5) {
        warnings.push(
            `${distro.name} erfordert mehr Linux-Erfahrung. Als Einsteiger könntest du auf Herausforderungen stoßen.`,
        );
    }
    ...
    if (profile.ramAmount && profile.ramAmount <= 4 && de.idleRAM > 500) {
        warnings.push(
            `${de.name} benötigt relativ viel RAM. Mit ${profile.ramAmount} GB könnte es eng werden.`,
        );
    }
    ...
    if (profile.hasNvidia && ['debian', 'fedora'].includes(distro.id)) {
        warnings.push(
            `NVIDIA-Treiber-Installation kann bei ${distro.name} zusätzliche Schritte erfordern.`,
        );
    }
    ...
    if (profile.needsStability > 7 && distro.releaseModel === 'rolling') {
        warnings.push(
            `${distro.name} ist eine Rolling-Release-Distribution. Updates können gelegentlich Probleme verursachen.`,
        );
    }
    ...
    if (profile.hasOldHardware && distro.minRAM > 2) {
        warnings.push(
            `${distro.name} könnte auf älterer Hardware langsam sein.`,
        );
    }
    ...
    if (
        profile.experienceLevel < 2 &&
        de.tilingSupport &&
        de.beginnerFriendly < 5
    ) {
        warnings.push(
            `${de.name} ist ein Tiling Window Manager und erfordert Einarbeitung.`,
        );
    }
    ...
    if (['tails', 'qubes'].includes(distro.id)) {
        if (profile.needsGaming > 3) {
            warnings.push(`${distro.name} ist nicht für Gaming geeignet.`);
        }
        warnings.push(
            `${distro.name} hat Einschränkungen für den täglichen Gebrauch.`,
        );
    }
...
export function calculateMatches(answers: UserAnswers): DistroMatch[] {
    ...
    if (profile.needsPrivacy < 8) {
        eligibleDistros = eligibleDistros.filter(
            (d) => !['tails', 'qubes'].includes(d.id),
        );
    }
...
export function calculateQuickPreview(answers: UserAnswers): DistroMatch[] {
    ...
    const hasRequired = requiredQuestions.every(
