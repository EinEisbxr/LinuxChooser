...
export function checkGameCompatibility(
...
): DealBreakerWarning | null {
    if (
        !selectedGames ||
        selectedGames.length === 0 ||
        selectedGames.includes('none')
    ) {
        return null;
    }
    ...
    if (hasAnticheat) {
        return {
            ...
            message: `Die folgenden Spiele haben Anti-Cheat-Systeme, die unter Linux nicht funktionieren: ${gameNames.join(', ')}. Diese Spiele können aktuell NICHT unter Linux gespielt werden.`,
            suggestion:
                'Du hast mehrere Optionen: 1) Dual-Boot mit Windows einrichten, 2) Cloud Gaming nutzen (GeForce NOW, Xbox Cloud), oder 3) auf diese Spiele verzichten.',
            affectedItems: gameNames,
            links: [
                {
                    text: 'ProtonDB - Spielekompatibilität prüfen',
                    url: 'https://www.protondb.com',
                },
                {
                    text: 'Are We Anti-Cheat Yet?',
    ...
    return {
        type: 'game',
        severity: 'warning',
        title: 'Einige Spiele haben eingeschränkte Kompatibilität',
        message: `Die folgenden Spiele könnten Probleme unter Linux haben: ${gameNames.join(', ')}.`,
        suggestion:
            'Prüfe die Kompatibilität auf ProtonDB vor der Umstellung. Mit Workarounds sind diese Spiele oft spielbar.',
        affectedItems: gameNames,
        links: [{ text: 'ProtonDB', url: 'https://www.protondb.com' }],
    };
}
...
export function checkSoftwareCompatibility(
    ...
    if (
        ...
        selectedSoftware.includes('none')
    ) {
        return null;
    }
    ...
    const criticalSoftware: {
        ...
        adobe: {
            ...
            alternative:
                'Alternativen: GIMP (Photoshop), Inkscape (Illustrator), Kdenlive/DaVinci Resolve (Premiere), Audacity (Audition). Adobe selbst läuft nicht nativ.',
            severity: 'critical',
        },
        'ms-office': {
            name: 'Microsoft Office',
            alternative:
                'Alternativen: LibreOffice (kostenlos), OnlyOffice, oder Microsoft 365 im Browser. Die Desktop-Version läuft nicht nativ.',
            severity: 'warning',
        },
        autodesk: {
            name: 'AutoCAD / Autodesk',
            alternative:
                'Alternativen: FreeCAD, LibreCAD, Blender (für 3D). AutoCAD läuft nicht unter Linux.',
            severity: 'critical',
        },
        itunes: {
            name: 'iTunes',
            alternative:
                'iTunes läuft nicht unter Linux. Für iPhone-Sync nutze libimobiledevice. Musik-Management: Rhythmbox, Strawberry.',
            severity: 'warning',
        },
        'specific-windows': {
            name: 'Spezifische Windows-Software',
            alternative:
                'Prüfe ob die Software unter Wine/Proton läuft (WineHQ AppDB). Viele Programme funktionieren, manche nicht.',
            severity: 'warning',
        },
    };
    ...
    return {
        ...
        title:
            maxSeverity === 'critical'
                ? 'Wichtig: Benötigte Software nicht verfügbar'
                : 'Hinweis: Software-Alternativen verfügbar',
        message: `Folgende Software läuft nicht nativ unter Linux: ${affectedSoftware.join(', ')}.`,
        suggestion: suggestions.join(' '),
        affectedItems: affectedSoftware,
        links: [
            {
                text: 'WineHQ AppDB - Kompatibilität prüfen',
                url: 'https://appdb.winehq.org',
            },
            {
                text: 'AlternativeTo - Software-Alternativen',
...
export function checkHardwareCompatibility(
    ...
    if (gpu === 'nvidia') {
        ...
        suggestions.push(
            'NVIDIA-Treiber erfordern manchmal manuelle Installation. Distributionen wie Pop!_OS, Linux Mint oder Nobara machen dies einfacher.',
        );
    }
    ...
    if (computerAge === 'vintage') {
        ...
        suggestions.push(
            'Wähle eine leichtgewichtige Distribution (MX Linux, antiX) mit XFCE oder LXQt. Vermeide ressourcenhungrige DEs wie GNOME oder KDE.',
        );
    }
    ...
    if (ram === '2gb') {
        ...
        suggestions.push(
            'Mit 2 GB RAM solltest du unbedingt eine leichte Distribution und einen minimalen Desktop wie XFCE, LXQt oder einen Tiling WM wählen.',
        );
    }
 ...
 */
export function checkCompetitiveGaming(
    answers: UserAnswers,
): DealBreakerWarning | null {
    ...
    return {
        ...
        message:
            'Viele kompetitive Multiplayer-Spiele nutzen Anti-Cheat-Software, die unter Linux nicht funktioniert. Dies betrifft besonders Spiele wie Valorant, Fortnite, und einige Call of Duty Titel.',
        suggestion:
            'Prüfe vor dem Wechsel auf areweanticheatyet.com ob deine Spiele unterstützt werden. Für nicht-kompatible Spiele kannst du Dual-Boot einrichten.',
