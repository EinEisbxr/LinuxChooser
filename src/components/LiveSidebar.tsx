...
export default function LiveSidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="live-sidebar desktop-only" aria-label="Live-Vorschau der Empfehlungen">
        <div className="sidebar-header">
          <h3 className="sidebar-title">
            <svg viewBox="0 0 24 24" fill="currentColor" className="sidebar-icon" aria-hidden="true">
        ...
        <div className="sidebar-content" aria-live="polite" aria-atomic="false">
          {hasLiveResults ? (
                ...
                {topResults.map((result, index) => (
                  ...
                  >
                    ...
                    <div className="result-info">
                      <span className="result-name">{result.distro.name}</span>
                      <span className="result-desktop">{result.de.name}</span>
                    </div>
            ...
            <div className="sidebar-placeholder">
              <svg viewBox="0 0 24 24" fill="currentColor" className="placeholder-icon" aria-hidden="true">
      ...
      <div className="live-sidebar-mobile mobile-only">
        ...
        >
          <div className="toggle-content">
            <svg viewBox="0 0 24 24" fill="currentColor" className="toggle-icon" aria-hidden="true">
              <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/>
            </svg>
            <span>Live-Vorschau</span>
            {hasLiveResults && (
              <span className="top-result-preview" aria-hidden="true">
            ...
            >
              {hasLiveResults ? (
                <ul className="results-list mobile">
                  {topResults.map((result, index) => (
                    <li key={`${result.distro.id}-${result.de.id}`} className="result-item">
                      <div className="result-rank">#{index + 1}</div>
                      {result.distro.logo && (
                        <img
                          src={result.distro.logo}
